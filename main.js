...
// sendDataToServer(uniquedetails, hash);

// function sendDataToServer(details, hash) {
//   const url = 'http://10.1.32.92:5000/api/unique-info';
//   const requestBody = {
//     ...details,
//     hash: hash,
//   };

//   console.log('Sending data to server:', requestBody); // Debugging payload
...
async function sendDataToServer(systemInfo) {
  try {
    const response = await fetch('http://10.1.32.92:5000/api/unique_info', {
      ...
      body: JSON.stringify({ud: systemInfo}),
    });
